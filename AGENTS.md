...
- The help screen's "every reachable key is documented" claim is proven by `internal/ui/help_completeness_test.go`, not luck. Dispatched keys outside `KeyMap` (`ctrl+c`, view digits, `` ` ``) are named constants beside the keymap in `internal/ui/keys.go` and shared by dispatch, `Reserved()` and the test; a new dispatch literal or `KeyMap` field fails the suite until it is classified in the test's `viewOnlyKeys` or excluded via `helpHiddenBindings` (one structure, why-comment per entry). `Reserved()` must cover every dispatched literal or help advertises custom commands that can never fire (digits 4/5 once slipped through exactly this way). Per-view help rows come only from branches of `helpBindings` for views whose dispatch really handles those verbs - ViewSystem is read-only and must not inherit the containers block (it once did, advertising dead stop/start/shell rows there).
