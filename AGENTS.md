...
- `Model.errDurable` (`internal/ui/app.go`) marks an action failure (set via `setActionErr`, used by the `actionDoneMsg` case) as surviving the containers refresh that verb's own handler triggers and any later periodic tick's refresh - `applyContainersLoaded`'s self-heal-on-success only clears `lastErr` when `!errDurable` (alongside the pre-existing services-down carve-out). It is cleared the normal way: any `setLastErr` call (a later action's success, a fresh load-originated error) resets it. A plain `setLastErr` (used for load-panel errors like a `containersLoadedMsg`/`imagesLoadedMsg`/`volumesLoadedMsg` failure) stays non-durable on purpose, since a later success of that *same* call is real evidence the problem resolved - only an action's failure has no such self-correcting signal from an unrelated background refresh.
