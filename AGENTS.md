...
- Custom commands bound to a key they can never fire on (reserved, unproducible spelling) or carrying no command are silently dropped from dispatch AND help by `customKey` (`internal/ui/keys.go`); `classifyCustomKey` in the same file resolves that exact predicate chain plus a reason, and the startup notice (`ignoredKeysNotice`, surfaced through `setStatus` in `newModel`) reports all of them once via the ordinary footer lifecycle. A missing key with a command present is the documented action-menu-only form and is deliberately never reported. Because `New()` can now set a startup status, ui-package tests must build models via `newModel(cfg)` / `newTestModel()`, not bare `New()`, when they assert footer or status state — bare `New()` reflects the host's dotfiles.
