...
- The help screen's "every reachable key is documented" claim is walked against rendered output by `internal/ui/help_completeness_test.go`, not luck. Dispatched keys outside `KeyMap` (`ctrl+c`, view digits, `` ` ``) are named constants beside the keymap in `internal/ui/keys.go` and shared by dispatch, `Reserved()` and the test; the test-side inventories (`dispatchedLiteralKeys`, `viewOnlyKeys`, `helpHiddenBindings` - one structure, why-comment per entry) are hand-synced rather than mechanically closed - no test reads handleKey's switch literals and a new `KeyMap` field classifies as global automatically - so review, not the suite, is what keeps those lists complete. `Reserved()` must cover every dispatched literal or help advertises custom commands that can never fire (digits 4/5 once slipped through exactly this way). Per-view help rows come only from branches of `helpBindings` for views whose dispatch really handles those verbs - ViewSystem is read-only and must not inherit the containers block (it once did, advertising dead stop/start/shell rows there).
- `internal/ui/networks/` is deliberately list-and-inspect only (no create/delete/prune) — the network view was scoped that way on purpose, to confirm the live `container network` JSON shape before any destructive action is built on it. It has no mark/multi-select support for the same reason (nothing to bulk-delete). Its tests, and `internal/backend/networks_test.go`, use `testify/suite` with camelCase scenario methods and named (non-table-driven) tests; so do `internal/ui/keys_notice_test.go` and the help-completeness walk (`internal/ui/help_completeness_test.go`). Every other test in this repo still uses plain `testing.T` — that split is intentional per the tasks that introduced suites, not drift to reconcile in either direction without asking.
