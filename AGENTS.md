...
- `Model.errDurable` (`internal/ui/app.go`) marks an action failure (set via `setActionErr`, used by the `actionDoneMsg` case) as surviving the containers refresh that verb's own handler triggers and any later periodic tick's refresh - `applyContainersLoaded`'s self-heal-on-success only clears `lastErr` when `!errDurable` (alongside the pre-existing services-down carve-out). It is cleared the normal way: any `setLastErr` call (a later action's success, a fresh error from any other path) resets it. Everything else stays non-durable, for two different reasons. Background load errors (`containersLoadedMsg`/`imagesLoadedMsg`/`volumesLoadedMsg`/`networksLoadedMsg`) are non-durable because a later success of that *same* call is real evidence the problem resolved. The other plain `setLastErr` sites - `shellDoneMsg`, `logsOpenedMsg` and the two clipboard-yank failures - are user-initiated and have no such self-correcting signal, so they are wiped by the next successful poll exactly like the `actionDoneMsg` bug that `errDurable` fixes; leaving them non-durable was a deliberate scope decision for that change, not evidence that durability is unwanted there.
